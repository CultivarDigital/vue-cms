...
router.get('/', (req, res) => {
  ...
  Preparation.find(query).sort('createdAt').exec((err, learningUnits) => {
    if (err) {
...
router.post('/', auth.authenticated, (req, res) => {
  ...
  newPreparation.site = req.payload.site
  newPreparation.user = req.payload.id

  newPreparation.save((err, learningUnit) => {
    if (err) {
      res.status(422).send(err.message)
    } else {
